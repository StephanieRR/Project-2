var actorChars = {
  ...
  'm': Mini, //interactive element; makes player small to reach certain areas in the game.
  'b': Big, // returns player back to normal size. Neither the Mini nor Big coins are made to disappear.
            // Both should be readily available for when Player wants to switch between both sizes.
...
// Collision detection for the actors is handled seperately from tiles.
...
var maxStep = 0.05;
var wobbleSpeed3 = 10;
var wobbleDist3 = 0.15;

Big.prototype.act = function(step) {
      this.wobble += step * wobbleSpeed2;
      var wobblePos = Math.sin(this.wobble) * wobbleDist2;
      this.pos = this.basePos.plus(new Vector(wobblePos,0));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    if(type == "mini") {
      this.player.size = new Vector(0.4,0.6);
    }

    if (type == "teleport") {
      this.player.pos = new Vector(6,20);
    }

    if (type == "big") {
      this.player.size = new Vector(0.8,1.5);
    }
    // Mini and Big coins do not disappear. Only alter the size of Player.
    //Teleport location is always a hazard in every level.
};
